...
const {find, mergeWith} = require('lodash');
const conventionalChangelogAngular = require('conventional-changelog-angular');
const HOSTS_CONFIG = require('./hosts-config');
 ...
 */
module.exports = async ({preset, config, parserOpts, writerOpts}, hostname) => {
  const hostDefaults = find(HOSTS_CONFIG, conf => conf.hostname === hostname) || HOSTS_CONFIG.default;
  let loadedConfig = {};
  ...
  if (preset) {
    ...
    if (!parserOpts) {
      loadedConfig.parserOpts = (await conventionalChangelogAngular).parserOpts;
    }
    if (!writerOpts) {
      loadedConfig.writerOpts = (await conventionalChangelogAngular).writerOpts;
    }
  }
